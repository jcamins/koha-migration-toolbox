...
if ($cat_year_map_name){
   my $csv = Text::CSV->new();
   open my $mapfile,"<$cat_year_map_name";
   while (my $row = $csv->getline($mapfile)){
      my @data=@$row;
      $patron_cat_map{$data[0]} = $data[1];
      $birthdate_hash{$data[0]} = $data[2];
   }
   close $mapfile;
}
...
while (my $line = $csv->getline($in)) {
   ...
   for (my $j=0;$j<scalar(@data);$j++){
      ...
      if ($fields[$j] =~ /^GROUP1/ ){
         ...
         if (exists $birthdate_hash{$data[$j]}){
            if ($birthdate_hash{$data[$j]} ne ""){
               $thisborrower{'birthdate'} = $birthdate_hash{$data[$j]};
            }
         }
      }
      if (($fields[$j] =~ /^GROUP2/ ) || ($fields[$j] =~ /^GROUP3/ )){
         if ($data[$j] eq "Youth" && $thisborrower{'categorycode'} eq "NRCHILD"){
            $thisborrower{'categorycode'}="NRYOUTH";
         }
         if ($data[$j] eq "Adult" && $thisborrower{'categorycode'} eq "NRCHILD"){
            $thisborrower{'categorycode'}="NRYOUTH";
         }
         if ($data[$j] =~ /(\d{4})/ ){
            my $year = $1;
            if ($year <= 1996){
               $thisborrower{'categorycode'} =~ s/CHILD/YOUTH/;
            }
            if ($year <= 1992){
               $thisborrower{'categorycode'} =~ s/YOUTH/ADULT/;
            }
            $thisborrower{'birthdate'} = sprintf "%4d-12-31",$year;
         }
      }
