...
while (my $line = $csv->getline($in)) {
   ...
   for (my $k=0;$k<scalar(@data);$k++){
      ...
      if ($fields[$k] eq "OUT"){
         $thisissue{'issuedate'} = substr($data[$k],6,4)."-".substr($data[$k],0,2)."-".substr($data[$k],3,2);
      }
      if ($fields[$k] eq "DUE"){
         $thisissue{'duedate'} = substr($data[$k],6,4)."-".substr($data[$k],0,2)."-".substr($data[$k],3,2);
      }
   }
