...
#   adapted accounts_loader to handle payments.
#---------------------------------
...
print "\n Borrower not found: $thisbar ";
   ...
   my $amount = ($data[6] - ($data[6] *2)) /100;

   my $transdesc;
   my $accounttype;
   if ($data[3] eq "Payment") {
      $transdesc = "Payment";
      $accounttype = 'C';
   }
   elsif ($data[3] eq "Forgive") {
      $transdesc = "Forgive";
      $accounttype = 'For';
   }
   elsif ($data[3] eq "Error") {
      $transdesc = "Error";
      $accounttype = 'For';
   }
   elsif ($data[3] eq "Refund") {
      $transdesc = "Refund";
      $accounttype = 'C';
   }
   else{
