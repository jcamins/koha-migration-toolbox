...
while () {
   ...
   foreach my $field ($record->field("852")){
      ...
      $itype{$barcode} = uc($field->subfield('6')) || "";
      $itype{$barcode} = "UNKNOWN" if ($itype{$barcode} eq "");
      my $orig_type = $itype{$barcode};
      my $prefix = $field->subfield('k') || '';
      if (exists($prefix_itype_map{$prefix})){
         $itype{$barcode} = $prefix_itype_map{$prefix};
      }
...
#BJCC- added line to place source of acqusition donation, etc in nonpublic note
     ...
     # if ($field->subfield('k') && ($type_code_map_name eq "")){
     #    $collcode{$barcode} = $field->subfield('k');
     #    if (exists($collcode_map{$collcode{$barcode}})){
     #       $collcode{$barcode} = $collcode_map{$collcode{$barcode}};
     #    }
     # }
     # if ($type_code_map_name ne "" && exists($collcode_map{$orig_type})){
     #    $collcode{$barcode} = $collcode_map{$orig_type};
     # }
      $collcodecount{$collcode{$barcode}}++ if $collcode{$barcode};

      my $mainpart = $field->subfield('h') || "";
      my $suffix = $field->subfield('i') || "";
      my $tail = $field->subfield('m') || "";
      $itemcall{$barcode} = $prefix." ".$mainpart." ".$suffix." ".$tail;
      $itemcall{$barcode} =~ s/^\s+//;
      $itemcall{$barcode} =~ s/\s+$//;
      
      if ($field->subfield('8')){
         my $year = substr($field->subfield('8'),0,4);
         my $month = substr($field->subfield('8'),4,2);
         my $day = substr($field->subfield('8'),6,2);
         if ($month && $day && $year){
