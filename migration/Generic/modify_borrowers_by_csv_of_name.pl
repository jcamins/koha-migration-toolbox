...
#      <borrower lastnamee>,<borrower firstname>,<new value>,<new value>...]
...
my $sth=$dbh->prepare("SELECT borrowernumber FROM borrowers WHERE surname = ? and firstname = ?");
...
FIELD:
   for my $j (0..scalar(@fields)-1) {
      ...
      $debug and print "$data[0], $data[1] ($rec->{borrowernumber})  $fields[$j] => $data[$j+2]\n";
      if ($doo_eet){
         my $update_sth =$dbh->prepare("UPDATE borrowers SET $fields[$j]=\"$data[$j+2]\" WHERE borrowernumber = $rec->{'borrowernumber'}");
         $update_sth->execute();
