...
my %thisrow;
my @charge_fields= qw{ borrowerbar itembar datedue issuedate branchcode };
...
#   next LINE if $line =~ /^s: /;
 ...
 print $outfl $borr.','.$itembar.',';
   ...
   if ($data[1] =~ m/[0-9]+\/[0-9]+\/[0-9]+/ ) {
       $datedue = format_the_date($data[1]);
       $issued = format_the_date($data[1]);
#$debug and print "$datedue and $issued\n";
       print $outfl $datedue.','.$issued.',VSPW'."\n";
       next LINE;
       $written++;
      }
}
