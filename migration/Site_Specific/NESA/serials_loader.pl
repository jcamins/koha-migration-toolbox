...
while (my $line=$csv->getline_hr($input_file)) {
   ...
   if ($doo_eet) {
      $sub_insert_sth->execute($default_librarian, $default_branchcode, $biblionumber, undef,                    1, 
                               $note,              $location,           $vendor,       $subscription_start_date, undef);
      my $subscription_id = $dbh->last_insert_id(undef, undef, undef, undef);
      if ($subscription_id) {
         $hist_insert_sth->execute($biblionumber, $subscription_id, '', "Migrated from Millenium");
         $written++;
      }
      else {
         $problem_2++;
