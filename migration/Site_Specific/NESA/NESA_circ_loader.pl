...
my $sth = $dbh->prepare("INSERT INTO issues (borrowernumber, itemnumber, date_due, branchcode, issuedate) VALUES (?, ?, ?, ?, NOW())");
...
while (my $line = $csv->getline($in))
{
   ...
   if ($data[0] ne "") {
      ...
      if ($borrnum) 
         {
         $doo_eet and $sth->execute($borrnum,$itemnum,$duedate,$branch);
